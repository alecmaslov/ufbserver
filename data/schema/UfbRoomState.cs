...
namespace UFB.StateSchema {
	public partial class UfbRoomState : Schema {
		...
		[Type(3, "map", typeof(MapSchema<PlayerState>))]
		public MapSchema<PlayerState> players = new MapSchema<PlayerState>();

		[Type(4, "ref", typeof(MapState))]
		...
		[Type(7, "ref", typeof(FooState))]
		public FooState foo = new FooState();
		...
		protected event PropertyChangeHandler<FooState> __fooChange;
		public Action OnFooChange(PropertyChangeHandler<FooState> __handler, bool __immediate = true) {
			if (__callbacks == null) { __callbacks = new SchemaCallbacks(); }
			__callbacks.AddPropertyCallback(nameof(this.foo));
			__fooChange += __handler;
			if (__immediate && this.foo != null) { __handler(this.foo, null); }
			return () => {
		...
		protected override void TriggerFieldChange(DataChange change) {
			switch (change.Field) {
				...
				case nameof(foo): __fooChange?.Invoke((FooState) change.Value, (FooState) change.PreviousValue); break;
				default: break;
			}
		}
	}
